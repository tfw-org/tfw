...
public final class Au
{
		...
		throws DataInvalidException, IOException
	{
		...
		if (byteIla.length() < MINIMUM_HEADER_LENGTH)
		{
			throw new IOException(
				"length < "+MINIMUM_HEADER_LENGTH+" not allowed!");
		}
		Argument.assertNotLessThan(byteIla.length(),
			MINIMUM_HEADER_LENGTH, "byteIla.length()");
		
		auByteData = byteIla;
		
		magicNumber = unsignedIntFromBytes(
			byteIla.toArray(0, 4), 0, SUN_MAGIC_NUMBER);
		...
		{
			throw new IOException(
				"offset("+offset+") < "+MINIMUM_HEADER_LENGTH+" not allowed!");
		}
		if (offset > byteIla.length())
		{
			throw new IOException(
				"offset("+offset+") > length("+byteIla.length()+") not allowed!");
		}
		...
		audioData = ByteIlaSegment.create(byteIla, offset,
			byteIla.length() - offset);
	 ...
	 */
	public Au(long encoding, long sampleRate, long numberOfChannels,
		ByteIla annotation, ByteIla audioData)
	{
		checkEncoding(encoding);
		Argument.assertGreaterThanOrEqualTo(sampleRate, 0, "sampleRate");
		Argument.assertGreaterThanOrEqualTo(
			numberOfChannels, 0, "numberOfChannels");
		Argument.assertNotNull(annotation, "annotation");
		Argument.assertEquals(
			annotation.length() % 8, 0, "annotation.length() % 8", "0");
		Argument.assertEquals(audioData.length() % numberOfChannels, 0,
			"audioData.length() % numberOfChannels", "0");
		...
		this.auByteData = ByteIlaConcatenate.create(
			ByteIlaConcatenate.create(
			ByteIlaFromArray.create(header), annotation), audioData);
	...
	private static long unsignedIntFromBytes(
		byte[]bytes, int offset, long magicNumber)
	{
