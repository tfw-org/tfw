...
public class IntIlmCircularCache {
	...
	private static class MyIntIlm extends AbstractIntIlm {
		...
		@Override
		protected synchronized void toArrayImpl(int[] array, int offset, int rowStride, int colStride, long rowStart,
			...
			if (cacheStart == 0 && cacheEnd == 0 || rowStart > cacheEnd || rowStart+rowCount < cacheStart) {
				...
				if (rowStart % maxRows == 0) {
					intIlm.toArray(buffer, rowStartOffset*rStride, rStride, cStride,
					...
					intIlm.toArray(buffer, first*rStride, rStride, cStride,
					...
					if (first > 0) {
						intIlm.toArray(buffer, 0, rStride, cStride,
			...
			if (cacheStart <= rowStart && rowStart <= cacheEnd) {
				...
				newRowCount = cacheEnd + newRowCount < intIlm.height() ? newRowCount :
					(int)(intIlm.height() - cacheEnd);
				
				if (newRowStartOffset + newRowCount <= maxRows) {
					intIlm.toArray(buffer,newRowStartOffset*rStride, rStride, cStride, cacheEnd,
					...
					intIlm.toArray(buffer,0,rStride,cStride,cacheEnd+countA,
			...
			if (cacheStart <= rowStart+rowCount && rowStart + rowCount <= cacheEnd) {
				...
				if (rowStartOffset + newRowCount <= maxRows) {
					...
					intIlm.toArray(buffer,end*rStride,rStride,cStride,rowStart,
							colStart,countA, colCount);
					
					if (countB > 0) {
						intIlm.toArray(buffer,0,rStride,cStride,rowStart+countA,
								colStart,countB,colCount);
					}
					else {
