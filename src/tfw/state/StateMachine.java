...
public class StateMachine
{
	...
	public static class Transition
	{
		...
		public Transition(Object trigger, Object fromState, Object toState,
			Guard guard, List<Action> actions)
		{
			...
			this.actions = (actions == null) ?
				null:
				Collections.unmodifiableList(new ArrayList<Action>(actions));
		}
	}
		...
		synchronized (lock)
		{
			...
			for ( ; i < transitions.size() ; i++)
			{
				...
				final boolean e = (t.trigger == event) ||
					t.trigger instanceof Class<?> &&
						((Class<?>)t.trigger).isInstance(event);
				final boolean g = t.guard == null ||
					(t.guard != null && t.guard.isTrue(event));
