...
public abstract class AbstractIlm implements ImmutableLongMatrix
{
    ...
    protected final void boundsCheck(int arrayLength, int offset, int rowStride,
    	int colStride, long rowStart, long colStart, int rowCount, int colCount)
    {
    	...
    	if (offset+(rowCount-1)*rowStride+(colCount-1)*colStride+1 > arrayLength) {
    		throw new IllegalArgumentException(
    			...
    			(offset+(rowCount-1)*rowStride+(colCount-1)*colStride+1) + ") > "+
    			"arrayLength" + " (="+arrayLength+") not allowed!");
    	}
    	...
    	if (rowStart + rowCount > height) {
    		throw new IllegalArgumentException(
    			"rowStart + rowCount > height" + " (="+(rowStart + rowCount > height) +
    			") > height not allowed!");
    	}
    	if (colStart + colCount > width) {
    		throw new IllegalArgumentException(
    			"colStart + colCount > width" + " (="+(colStart + colCount > width) +
    			") > width not allowed!");
    	}
