...
public final class StringIlaCheck {
    ...
    public static void checkAll(
            StringIla target, StringIla actual, int addlOffsetLength, int maxAbsStride, String epsilon)
            throws Exception {
    ...
    public static void checkWithoutCorrectness(StringIla ila, int offsetLength, String epsilon) throws Exception {
    ...
    public static void checkZeroArgImmutability(StringIla ila) throws Exception {
    ...
    public static void checkTwoArgImmutability(StringIla ila, String epsilon) throws Exception {
        if (epsilon != "") throw new IllegalArgumentException("epsilon != " + ("") + " not allowed");
        else {
    ...
    public static void checkTwoFourEquivalence(StringIla ila, String epsilon) throws Exception {
        if (epsilon != "") throw new IllegalArgumentException("epsilon != " + ("") + " not allowed");
        else {
            final int ilaLength = ila.length() <= Integer.MAX_VALUE ? (int) ila.length() : Integer.MAX_VALUE;
    ...
    public static void checkFourFiveEquivalence(StringIla ila, int offsetLength, String epsilon) throws Exception {
        if (epsilon != "") throw new IllegalArgumentException("epsilon != " + ("") + " not allowed");
        else {
                        ...
                        for (int length = 1; length <= ilaLength; ++length) {
                            for (long start = 0; start < ilaLength - length + 1; ++start) {
                                for (int ii = 0; ii < targetBase.length; ++ii) {
                                    targetBase[ii] = actualBase[ii] = new String();
                                }
