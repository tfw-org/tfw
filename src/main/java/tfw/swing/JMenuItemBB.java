...
public class JMenuItemBB extends JMenuItem implements BranchBox {
    ...
    public JMenuItemBB(Branch branch, StatelessTriggerECD triggerECD, BooleanECD enabledECD) {
        this.branch = branch;

        ActionInitiator actionInitiator = new ActionInitiator("JMenuItemBB", triggerECD);
        addActionListener(actionInitiator);
        branch.add(actionInitiator);

        if (enabledECD != null) {
            EnabledCommit enabledCommit = new EnabledCommit("JMenuItemBB", enabledECD, this, null);
