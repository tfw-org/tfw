...
public class MultiplexedBranch extends BranchComponent {
    private final Map<TreeComponent, Object> slotIdFromChild = new HashMap<TreeComponent, Object>();
    private final Map<Object, Branch> subBranchFromSlotId = new HashMap<Object, Branch>();
     ...
     */
    public final synchronized void add(TreeComponent child, int multipexIndex) {
    ...
    public final synchronized void add(TreeComponent child, Object slotId) {
        Argument.assertNotNull(child, "child");
    ...
    public final synchronized void add(BranchBox branchBox, Object slotId) {
    ...
    public final synchronized void remove(TreeComponent child) {
        ...
        if (slotId == null) {
            throw new IllegalStateException(
                    "Attempt to remove '" + child.getName() + "(" + child + ")' for which no slot identifier exists");
        }

        Branch subBranch = subBranchFromSlotId.get(slotId);
        if (subBranch == null) {
            throw new IllegalStateException(
                    "Attempt to remove '" + child.getName() + "(" + child + ")' for which no sub-branch exists");
        }

        slotIdFromChild.remove(child);
        subBranch.remove(child);

        if (subBranch.immediateChildren == null || subBranch.immediateChildren.size() == 0) {
            removeChild(subBranch);
            subBranchFromSlotId.remove(slotId);
        }
    }

    public final synchronized void remove(TreeComponent[] children) {
    ...
    public final synchronized void removeAll(Object slotId) {
        Branch subBranch = subBranchFromSlotId.get(slotId);
        if (subBranch != null) {
