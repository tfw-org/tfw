...
public class ByteIlmCoreCircularCache {
	...
	public static ByteIlm create(ByteIlm byteIlm, int numRows,
			ByteIlm byteIlmCoreCircularCache) {
	...
	private static class MyByteIlm extends AbstractByteIlm {
		...
		@Override
		protected void toArrayImpl(byte[] array, int offset, int rowStride, int colStride, long rowStart, long colStart,
	...
	private static class Core {
		...
		public void toArrayImpl(byte[] array, int offset, int rowStride, int colStride, long rowStart,
			...
			synchronized (lock) {
				...
				if (cacheStart == 0 && cacheEnd == 0 || rowStart > cacheEnd || rowStart+rowCount < cacheStart) {
					...
					if (rowStart % maxRows == 0) {
						byteIlm.toArray(buffer, rowStartOffset*rStride, rStride, cStride,
						...
						byteIlm.toArray(buffer, first*rStride, rStride, cStride,
						...
						if (first > 0) {
							byteIlm.toArray(buffer, 0, rStride, cStride,
				...
				if (cacheStart <= rowStart && rowStart <= cacheEnd) {
					...
					newRowCount = cacheEnd + newRowCount < byteIlm.height() ? newRowCount :
						(int)(byteIlm.height() - cacheEnd);
					
					if (newRowStartOffset + newRowCount <= maxRows) {
						byteIlm.toArray(buffer,newRowStartOffset*rStride, rStride, cStride, cacheEnd,
						...
						byteIlm.toArray(buffer,0,rStride,cStride,cacheEnd+countA,
				...
				if (cacheStart <= rowStart+rowCount && rowStart + rowCount <= cacheEnd) {
					...
					if (rowStartOffset + newRowCount <= maxRows) {
						...
						byteIlm.toArray(buffer,end*rStride,rStride,cStride,rowStart,
								colStart,countA, colCount);
						
						if (countB > 0) {
							byteIlm.toArray(buffer,0,rStride,cStride,rowStart+countA,
									colStart,countB,colCount);
						}
						else {
