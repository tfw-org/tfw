...
public final class BranchProxy implements Proxy {
    ...
    public EventChannelProxy[] getEventChannelProxies() {
        ...
        EventChannelProxy[] eventChannelProxies = new EventChannelProxy[eventChannels.length];

        for (int i = 0; i < eventChannels.length; i++) {
            eventChannelProxies[i] = new EventChannelProxy((EventChannel) eventChannels[i]);
        }
    ...
    public Proxy[] getChildProxies() {
        Map<String, TreeComponent> children = new TreeMap<String, TreeComponent>(branch.getChildren());
        ...
        for (int i = 0; i < proxies.length; i++) {
            ...
            if (treeComponent instanceof Root) {
                ...
                proxies[i] = new MultiplexedBranchProxy((MultiplexedBranch) treeComponent);
                ...
                proxies[i] = new TriggeredConverterProxy((TriggeredConverter) treeComponent);
