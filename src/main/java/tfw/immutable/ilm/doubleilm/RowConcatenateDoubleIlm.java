	...
	private static class MyDoubleIlm extends AbstractDoubleIlm {
		...
		protected void toArrayImpl(double[] array, int offset, int rowStride, int colStride, long rowStart,
			...
			if (colStart + colCount <= leftIlm.width()) {
				...
				rightIlm.toArray(array, offset, rowStride, colStride, rowStart, colStart - leftIlm.width(), rowCount, colCount);
			}
			else {
				...
				rightIlm.toArray(array, offset + firstAmount, rowStride, colStride, rowStart, 0, rowCount, colCount - firstAmount);
			}
		}
	}
}
