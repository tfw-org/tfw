...
public final class IntIlaInterleave {
    ...
    public static IntIla create(IntIla[] ilas, final int[] buffer) {
        return new IntIlaImpl(ilas, buffer);
    }
    ...
    private static class IntIlaImpl extends AbstractIntIla {
        ...
        private IntIlaImpl(IntIla[] ilas, final int[] buffer) {
            ...
            for (int i = 0; i < ilas.length; i++) {
                stridedIntIlas[i] = StridedIntIlaFromIntIla.create(ilas[i], buffer.clone());
            }
        }
        ...
        protected void closeImpl() throws IOException {
            for (StridedIntIla ila : stridedIntIlas) {
                ila.close();
            }
        }
    }
