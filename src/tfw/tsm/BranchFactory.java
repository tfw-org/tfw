...
public class BranchFactory extends BaseBranchFactory
{
     ...
     *
     * @param childEventChannel the child event channel.
     * @param parentEventChannel the parent event channel.
     */
        ...
        EventChannelDescription parentEventChannel)
    {
		...
		if (!childEventChannel.getConstraint().isCompatable(parentEventChannel.getConstraint()))
		{
		...
		if (!parentEventChannel.getConstraint().isCompatable(childEventChannel.getConstraint()))
		{
			throw new IllegalArgumentException(
    ...
    /**
     *  Adds a terminator for the specified event channel. A terminator is
     * the point within the component tree structure where an event channel
     * stops. Events from sources or publishers move up through the component
     * hierarchy until they reach the terminator. If the event represents a
     * state change as defined by the {@link StateChangeRule} the terminator
     * reflects the event back down through the component tree structure to
     * the sinks or subscribers.
     *
     * @param eventChannelDescription The event channel to be terminated.
     * @param initialState The initial value to be assigned to the event
     * channel.
    ...
    /**
     * Creates a {@link Branch} with the previously specified translators
     * and terminators.
     * @param name The name of the branch.
     * @return A branch with the previously specified translators and terminators.
     */
    public Branch create(String name)
    {
        return new Branch(name, getSinks(), getSources(), getTerminators());
    }
}
