...
public class EdgeToGraphicConverter extends Converter {
    ...
    public EdgeToGraphicConverter(GraphECD graphECD, IntIlmECD pixelNodeTLBRECD, GraphicECD graphicECD) {
        super("EdgeToGraphicConverter", new ObjectECD[] {graphECD, pixelNodeTLBRECD}, null, new ObjectECD[] {graphicECD
        });
    ...
    protected void convert() {
        ...
        for (int i = 0; i < froms.length; i++) {
            if (froms[i] != null) {
                ...
                graphic = DrawLineGraphic.create(
                        graphic,
                        lefts[from] + (rights[from] - lefts[from]) / 2,
                        bottoms[from],
                        lefts[to] + (rights[to] - lefts[to]) / 2,
                        tops[to]);
            }
        }
