...
public class DoubleIlmCircularCache {
	...
	private static class MyDoubleIlm extends AbstractDoubleIlm {
		...
		@Override
		protected synchronized void toArrayImpl(double[] array, int offset, int rowStride, int colStride, long rowStart,
			...
			if (cacheStart == 0 && cacheEnd == 0 || rowStart > cacheEnd || rowStart+rowCount < cacheStart) {
				...
				int rowsToGet = doubleIlm.height() > rowStart + maxRows ? maxRows : (int)(doubleIlm.height()-rowStart);
				...
				if (rowStart % maxRows == 0) {
					doubleIlm.toArray(buffer, rowStartOffset*rStride, rStride, cStride,
							rowStart, colStart, rowsToGet, colCount);
				}
				else {
					...
					doubleIlm.toArray(buffer, first*rStride, rStride, cStride,
							rowStart, colStart, firstLength, colCount);
					
					if (firstLength+first > rowCount) {
						first = rowCount-firstLength;
					}
					if (first > 0) {
						doubleIlm.toArray(buffer, 0, rStride, cStride,
								rowStart+firstLength,colStart, first, colCount);
					}
				}
			...
			if (cacheStart <= rowStart && rowStart <= cacheEnd) {
				...
				newRowCount = cacheEnd + newRowCount < doubleIlm.height() ? newRowCount :
					(int)(doubleIlm.height() - cacheEnd);
				
				if (newRowStartOffset + newRowCount <= maxRows) {
					doubleIlm.toArray(buffer,newRowStartOffset*rStride, rStride, cStride, cacheEnd,
							colStart, newRowCount, colCount);
				}
				else {
					...
					doubleIlm.toArray(buffer,newRowStartOffset*rStride, rStride, cStride,cacheEnd,
							colStart, countA, colCount);
					doubleIlm.toArray(buffer,0,rStride,cStride,cacheEnd+countA,
							colStart,countB,colCount);
				}
			...
			if (cacheStart <= rowStart+rowCount && rowStart + rowCount <= cacheEnd) {
				...
				if (rowStartOffset + newRowCount <= maxRows) {
					doubleIlm.toArray(buffer,rowStartOffset*rStride,rStride,cStride,
							cacheStart-newRowCount,colStart,newRowCount,colCount);
				}
				else {
					...
					doubleIlm.toArray(buffer,end*rStride,rStride,cStride,rowStart,
							colStart,countA, colCount);
					
					if (countB > 0) {
						doubleIlm.toArray(buffer,0,rStride,cStride,rowStart+countA,
								colStart,countB,colCount);
					}
					else {
